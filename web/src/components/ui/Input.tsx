...
const Input: React.FC<InputProps> = ({ value, onChange, placeholder, type = "text", password }) => {
  ...
  return (
    <div className="relative w-full">
     ...
     className='w-full p-3 bg-transparent border-b-2 border-gray-300 dark:border-gray-600 focus:outline-none focus:border-blue-500 transition-all duration-300 text-gray-900 dark:text-gray-100'
     />
     {
        password && (
        ...
        tabIndex={-1}
        >
            {showPassword ? <EyeOff className="w-6 h-6 text-blue-600 dark:text-white" /> : <Eye className="w-6 h-6 text-blue-600 dark:text-white" />}
        </button>
        )
     }
