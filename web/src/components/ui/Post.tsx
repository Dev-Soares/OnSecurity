...
const Post: React.FC<PostProps> = ({ username, content, timestamp, userAvatarUrl, imgUrl }) => {
  ...
  return (
    <div className='bg-gray-100 dark:bg-gray-800 dark:border-gray-700 border-2 border-gray-300 h-auto px-6 flex flex-col rounded-md gap-2 w-full pt-8 md:w-[70%] xl:w-[50%] shadow-lg'>
        <div className='flex flex-row gap-2 justify-start items-center border-t border-t-gray-300 dark:border-t-gray-700 py-4'>
            ...
            <div className='flex flex-col'>
                <p className='font-bold text-lg text-blue-500 dark:text-white'>{username}</p>
                <p className='text-sm text-gray-500 dark:text-gray-300'>{timestamp}</p>
            </div>
        </div>
        <div className='flex flex-col gap-4'>
            <p className='text-md font-medium text-gray-800 dark:text-gray-100'>{content}</p>
