...
const ArticleCard: React.FC<ArticleCardProps> = ({ title, description, bgUrl, slug }) => {
    ...
    return (
        <button onClick={() => navigate(`/articles/${slug}`)} className={`w-full h-75 md:h-85 xl:h-90 bg-cover bg-center rounded-lg shadow-lg bg-gray-100 dark:bg-gray-900 bg-opacity-50 overflow-hidden flex flex-col justify-end group cursor-pointer hover:shadow-xl hover:translate-y-[-4px] transition-all duration-300 max-w-md`}>
